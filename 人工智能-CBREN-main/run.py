...
parser.add_argument('--input_path', type=str,
                    default='/media/abc/学习1/Program/人工智能-CBREN-main/datasets/hevc_cbr_low_mp4/D/BasketballPass_416x240_50/',
                    help='input path to use')
parser.add_argument('--output_path', default='/media/abc/学习1/Program/人工智能-CBREN-main/datasets/hevc_cbr_low_mp4/D/BasketballPass_416x240_50', type=str,
...
def save_image_tensor(_input_tensor: torch.Tensor, filename):
    assert (len(_input_tensor.shape) == 4 and _input_tensor.shape[0] == 1)
    # 复制一份
    _input_tensor = _input_tensor.clone().detach()
    _input_tensor = _input_tensor[:, [2, 1, 0], :, :]
    # 到cpu
    _input_tensor = _input_tensor.to(torch.device('cpu'))
    vutils.save_image(_input_tensor, filename)


def save_image_tensor2cv2(_input_tensor: torch.Tensor, filename):
    assert (len(_input_tensor.shape) == 4 and _input_tensor.shape[0] == 1)
    # 复制一份
    _input_tensor = _input_tensor.clone().detach()
    # 到cpu
    _input_tensor = _input_tensor.to(torch.device('cpu'))
    # 去掉批次维度
    _input_tensor = _input_tensor.squeeze()
    # 从[0,1]转化为[0,255]，再从CHW转为HWC，最后转为cv2
    _input_tensor = _input_tensor.mul_(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).type(torch.uint8).numpy()
    # RGB转BRG
    # input_tensor = cv.cvtColor(input_tensor, cv.COLOR_RGB2BGR)
    cv.imwrite(filename, _input_tensor)


def save_image_tensor2pillow(_input_tensor: torch.Tensor, filename):
    assert (len(_input_tensor.shape) == 4 and _input_tensor.shape[0] == 1)
    # 复制一份
    _input_tensor = _input_tensor.clone().detach()
    # 到cpu
    ...
    _input_tensor = _input_tensor.mul_(255).add_(0.5).clamp_(0, 255).permute(1, 2, 0).type(torch.uint8).numpy()
    # 转成pillow
    im = Image.fromarray(_input_tensor)
    im.save(filename)


def padding8(img):
    _h, _w = img.shape[0:2]
    pad_h = 8 - _h % 8 if _h % 8 != 0 else 0
    pad_w = 8 - _w % 8 if _w % 8 != 0 else 0
    ...
    for i in listdir(input_path):
        if is_image_file(i):
            with torch.no_grad():
                ...
                # 获取图像尺寸
                height, width = input_center_img.shape[:2]
                # 设置合适的 win_size，确保它小于等于图像的最小维度，且是奇数
                win_size = min(height, width)
                if win_size % 2 == 0:  # 确保是奇数
                    win_size -= 1
                input_ssim = ssim(input_center_img, img_original, multichannel=True, win_size=win_size, channel_axis=2)
                output_ssim = ssim(out_img, img_original, multichannel=True, win_size=win_size, channel_axis=2)
